...
module.exports = function (RED) {
    ...
    function Subscription(n) {
        ...
        this.config = RED.nodes.getNode(n.config);
        let node = this;
        let YoutubeAPI = this.config ? this.config.YoutubeAPI : null;
        ...
        if (!YoutubeAPI) {
            ...
            node.status({
                fill: "red",
                shape: "ring",
                text: "Missing Google credentials"
            });
            return;
        }
        ...
        node.on("input", (msg) => {
            // Required =======================================================
            this.part = n.part || msg.part || "snippet"; // Acceptable: contentDetails, id, snippet, subscriberSnippet

            // exactly one of the following
            this.channelId = n.channelId || msg.channelId || "";
            this.subscriptionId = n.subscriptionId || msg.subscriptionId || "";
            /* NOTE: only possible w/ OAuth
            this.mine = n.mine || msg.mine || "";
            this.myRecentSubscribers = n.myRecentSubscribers || msg.myRecentSubscribers || "";
            this.mySubscribers = n.mySubscribers || msg.mySubscribers || "";
            */

            // Optional =======================================================
            this.maxResults = n.maxResults || msg.maxResults || "5"; // Acceptable: 0-50
            this.order = n.order || msg.order || "relevance"; // Acceptable: alphabetical, relevance, unread
            this.pageToken = n.pageToken || msg.pageToken || "";

            let json = { part: this.part };
            ...
            if (this.pageToken)
                json.pageToken = this.pageToken;

            node.log(JSON.stringify(json));
            ...
            YoutubeAPI.subscriptions.list(json, (err, data) => {
                if (err) {
                    node.error("Failed to fetch YouTube Subscriptions: " + JSON.stringify(err));
                    node.status({
                        fill: "red",
                        shape: "ring",
                        text: "error"
    ...
    RED.nodes.registerType("YouTube Subscriptions", Subscription)
